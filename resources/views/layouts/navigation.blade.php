<nav id="main-navbar" x-data="{ open: false, userOpen: false }" class="navbar">
    ...
    <a href="{{ route('dashboard') }}" style="display:flex;align-items:center;gap:10px;text-decoration:none;flex-shrink:0;">
        <div style="width:32px;height:32px;border-radius:9px;background:#1E45FB;display:flex;align-items:center;justify-content:center;font-size:1rem;box-shadow:0 4px 12px rgba(30,69,251,0.3);">⚡</div>
        <span style="font-size:1.1rem;font-weight:800;color:#0F172A;font-family:'Plus Jakarta Sans',sans-serif;">LevelUp</span>
    </a>
    ...
    <div class="hidden sm:flex items-center" style="gap:1rem;">
        @auth
                        ...
                        <form method="POST" action="{{ route('notifications.read', $notif->id) }}" style="display:block;">
                            ...
                            <button type="submit" style="display:flex;align-items:flex-start;gap:10px;padding:12px 16px;width:100%;text-align:left;background:rgba(30,69,251,0.03);border:none;border-bottom:1px solid rgba(30,69,251,0.04);cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(30,69,251,0.07)'" onmouseout="this.style.background='rgba(30,69,251,0.03)'">
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    <div style="font-size:.75rem;color:#475569;line-height:1.4;white-space:normal;">{{ $notif->data['message'] ?? '' }}</div>
            ...
            <span style="font-size:0.7rem;font-weight:700;padding:3px 10px;border-radius:999px;text-transform:uppercase;letter-spacing:0.06em;
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" style="display:flex;align-items:center;gap:8px;padding:5px 12px 5px 5px;border-radius:12px;background:#F8FAFC;border:1px solid rgba(30,69,251,0.08);cursor:pointer;transition:all .2s;" onmouseover="this.style.borderColor='rgba(30,69,251,0.2)'" onmouseout="this.style.borderColor='rgba(30,69,251,0.08)'">
                    <div style="width:28px;height:28px;border-radius:8px;background:#1E45FB;display:flex;align-items:center;justify-content:center;font-size:0.85rem;font-weight:700;color:#FFFFFF;">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                    <span style="font-size:0.85rem;font-weight:500;color:#0F172A;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ Auth::user()->name }}</span>
    ...
    {{-- Mobile Hamburger --}}
    <button @click="open = !open" class="sm:hidden" style="width:36px;height:36px;border-radius:10px;background:#F8FAFC;border:1px solid rgba(30,69,251,0.08);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;">
    ...
    {{-- Mobile Menu --}}
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
        style="position:fixed;top:78px;left:8px;right:8px;background:#FFFFFF;border:1px solid rgba(30,69,251,0.1);border-radius:16px;padding:1rem;box-shadow:0 20px 40px rgba(30,69,251,0.08);z-index:99;" class="sm:hidden">
        @auth
            @if(auth()->user()->isAdmin())
                <a href="{{ route('admin.dashboard') }}" class="nav-link" style="display:block;margin-bottom:4px;">Dashboard</a>
                <a href="{{ route('admin.users.index') }}" class="nav-link" style="display:block;margin-bottom:4px;">Pengguna</a>
                <a href="{{ route('admin.missions.index') }}" class="nav-link" style="display:block;margin-bottom:4px;">Moderasi</a>
            @elseif(auth()->user()->isMahasiswa())
                <a href="{{ route('mahasiswa.dashboard') }}" class="nav-link" style="display:block;margin-bottom:4px;">Dashboard</a>
                <a href="{{ route('mahasiswa.missions.browse') }}" class="nav-link" style="display:block;margin-bottom:4px;">Cari Mission</a>
                <a href="{{ route('mahasiswa.applications.index') }}" class="nav-link" style="display:block;margin-bottom:4px;">Lamaran</a>
                <a href="{{ route('mahasiswa.portfolio.index') }}" class="nav-link" style="display:block;margin-bottom:4px;">Portfolio</a>
                <a href="{{ route('profiles.show', auth()->user()) }}" class="nav-link" style="display:block;margin-bottom:4px;">Lihat Profil</a>
                <a href="{{ route('mahasiswa.profile.edit') }}" class="nav-link" style="display:block;margin-bottom:4px;">Edit Profil</a>
            @elseif(auth()->user()->isUmkm())
                <a href="{{ route('umkm.dashboard') }}" class="nav-link" style="display:block;margin-bottom:4px;">Dashboard</a>
                <a href="{{ route('umkm.missions.index') }}" class="nav-link" style="display:block;margin-bottom:4px;">Mission Saya</a>
                <a href="{{ route('umkm.missions.create') }}" class="nav-link" style="display:block;margin-bottom:4px;">Buat Mission</a>
                <a href="{{ route('profiles.show', auth()->user()) }}" class="nav-link" style="display:block;margin-bottom:4px;">Lihat Profil Bisnis</a>
                <a href="{{ route('umkm.profile.edit') }}" class="nav-link" style="display:block;margin-bottom:4px;">Edit Profil Bisnis</a>
            @endif
            <div style="height:1px;background:rgba(30,69,251,0.06);margin:10px 0;"></div>
            <div style="padding:0 12px;margin-bottom:8px;">
                <div style="font-size:0.85rem;font-weight:600;color:#0F172A;">{{ Auth::user()->name }}</div>
                <div style="font-size:0.75rem;color:#475569;">{{ Auth::user()->email }}</div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="nav-link" style="display:block;width:100%;text-align:left;color:#EF4444;">Keluar</button>
            </form>
        @else
            <a href="{{ route('login') }}" class="nav-link" style="display:block;margin-bottom:4px;">Masuk</a>
            <a href="{{ route('register') }}" class="btn btn-primary" style="display:block;text-align:center;margin-top:8px;">Daftar Gratis</a>
        @endauth
    </div>
</nav>
